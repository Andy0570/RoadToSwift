...
启动 Charles。它应该询问是否允许自动配置你的网络设置。如果没有，按 Shift-Command-P 就可以看到这个提示。
...
默认情况下，Charles 会跟踪你的 Mac 上的所有网络调用。现在，你只对一个特定的主机感兴趣：https://swapi.dev/。你可以通过只关注这个域名来避免在 Charles 中看到每个网络请求的干扰。
